...
{                                                                                           \
  ...
  __asm__ __volatile__ (                                                                    \
    ...
    "mov     r1, LO(%[ulCriticalNesting])\n\t"                                              \
    "orh     r1, HI(%[ulCriticalNesting])\n\t"                                              \
    "ld.w    r2, r1[0]\n\t"                                                                 \
    "cp.w    r2, 0\n\t" /* should use portNO_CRITICAL_NESTING instead, but leads to error */\
    "breq    LABEL_RET_SCALL_%[LINE]\n\t"                                                   \
    "sub     r2, r2, 1\n\t"                                                                 \
    "st.w    r1[0], r2\n\t"                                                                 \
    "cp.w    r2, 0\n\t" /* should use portNO_CRITICAL_NESTING instead, but leads to error */\
    "brne    LABEL_RET_SCALL_%[LINE]\n\t"                                                   \
    "cbr     r0, 16\n"                                                                      \
    "LABEL_RET_SCALL_%[LINE]:"                                                              \
    "st.w    r7[-2*4], r0\n\t"                                                 /* Copy SR */\
    "ld.w    r0, r7[-7*4]\n\t"                                                 /* Read PC */\
    "st.w    r7[-1*4], r0\n\t"                                                 /* Copy PC */\
                                                                                            \
 ...
 *
 * This macro only causes the function to be defined as an interrupt service
 * routine, i.e., it does not add any initialization code.
 *
 * Usage:
 * \code
 ...
 * IAR toolchain (avr32/io\<part\>.h) (Ignore it on GCC).
 *
