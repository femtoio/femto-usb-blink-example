 ...
 *
 */
/** \defgroup group_avr32_components_wifi_hd wl_api Reference Manual
...
#define WL_API_RELEASE_NAME "v2.6"
...
/*! Size of firmware image, as defined in wl_fw.h */
#define WL_FW_LEN            154188
...
\code
if ( wl_fw_download(NULL, fw_download_cb, &mode) != WL_SUCCESS ) {
 ...
 *         \a wl_fw_download() was used to download the firmware image to the
 ...
 *
 * \a wl_fw_download() and wl_init() must be invoked to startup the library
 * again.
 *
 * @return
 ...
 *
 * This function should read a number of bytes of the firmware
 ...
 * downloaded. wl_init() should be called after this function has return to
 ...
 *                       invalid.
 */
wl_err_t wl_fw_download(void *ctx, wl_fw_download_cb_t *fw_download_cb,
                        enum wl_host_attention_mode *mode);
 ...
 *
 * In systems using an OS, a task could be scheduled to run this task when a
 * new interrupt is signalled from the hardware.
 *
...
/*! \brief Register management tx callback functionWakeup callback function.
 *
