		...
		unsigned int nch, unsigned int ns)
{
  ...
  const mad_fixed_t /*const*/ (*sbsample)[36][32];
#  if defined(FPM_AVR32)
  signed short *pcm1;
#  else
  mad_fixed_t *pcm1;
  unsigned int sb, pe, po;
  mad_fixed_t *pcm2;
...
#  endif
  ...
  for (ch = 0; ch < nch; ++ch) {
    ...
    for (s = 0; s < ns; ++s) {
#  if defined(FPM_AVR32)
  extern void dct32_avr32(mad_fixed_t const in[32], unsigned int slot, \
                          mad_fixed_t lo[16][8], mad_fixed_t hi[16][8]);
  extern signed short *synth_avr32(unsigned int phase, mad_fixed_t const *filter, \
                                  signed short *pcm, short const *Dmod );
/*
  int i;
  for ( i = 0; i < 32; i++ ){
